...
/**
 * In charge of taking in a Map of Sprite's and updating them on each step,
 * according to present physics effects TODO: Possibly name this something
 * different
 * 
 * @author Krista
 *
...
public class ElementUpdater {
	/**
	 * This method updates each sprite's Position before Events (causes and
	 * effects) are applied
	 * 
	 */
	public void update(ILevelData leveldata) {
		...
		elements.stream().forEach((elempair) -> {
			...
			if (elem instanceof Sprite) {
				if (!(Boolean) ((Sprite) elem).getProperty(VoogaBundles.spriteProperties.getString("ALIVE"))
						.getValue()) {
	...
	private void applyGravity(Sprite e, IPhysicsEngine physics) {
		double gravityMagnitude = (double) ((Sprite) e).getProperty(VoogaBundles.spriteProperties.getString("GRAVITY"))
				.getValue();
