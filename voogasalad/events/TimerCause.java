...
public class TimerCause extends Cause {
    ...
    /**
     * Checks whether game time is past start time, and returns true periodically based on the
     * interval
     */
    @Override
    public boolean check (ILevelData data) {
        Double time =
                (Double) data.getGlobalVar(VoogaBundles.defaultglobalvars.getProperty("Time"))
                        .getValue();
