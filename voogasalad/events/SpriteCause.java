...
public class SpriteCause extends VariableCause {
	 ...
	 * 
	 * @param spriteID
	 *            ID of the Sprite being checked
	 * @param varName
	 *            Name of the variable of that Sprite that we're interested in
	 * @param predicate
	 *            A predicate method to be applied to the Sprite proeprty
	 * @param voogaevent
	 *            Event that this cause belongs to
	 */
	 ...
	 * @param predicate
	 * @param target
	 *            A target value for testing equality or other logical
	 *            relationships to the Sprite property
	 * @param voogaEvent
	 */
	...
	/**
	 * Separate constructor to take a Double instead of a Boolean. Needed for
	 * Reflection in factory.
	 * 
	 * @param spriteID
	 * @param varName
	 * @param predicate
	...
	public boolean check(ILevelData data) {
		...
		if (mySpriteID.matches("[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}")) {
			mySprites.add(data.getSpriteByID(mySpriteID)); // If contains dash,
															// it's a Sprite ID
		} else {
			mySprites.addAll(data.getSpritesByArch(mySpriteID));// Else, it's an
																// arch name
		}
	...
	public String toString() {
		return "Checking if the variable " + myVarName + " in Sprite " + mySpriteID + " is equal to "
				+ super.getTarget();
	}
}
