...
/**
 * 
 * @author Saumya Jain This class detects collisions between Sprites
 *
 */
...
public class CollisionCause extends Cause {
	...
	/**
	 * @param spriteA
	 *            Archetype or ID of a Sprite in the collision
	 * @param spriteB
	 *            Archetype or ID of a different Sprite
	 * @param voogaEvent
	 *            An event that this cause triggers
	 */
	 ...
	 *            An event that this cause triggers
	 * @param direction
	 *            The direction of the collision (Above, Below, Horizontal)
	 */
	 ...
	 * 
	 * @param data
	 *            A LevelData object containing Sprites
	 * @param arch
	 *            A spriteID or archetype name
	 * @return
	 */
	private List<Sprite> getSprites(ILevelData data, String arch) {
		...
		if (arch.matches("[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}")) {
			group.add(data.getSpriteByID(arch)); // If contains dash, it's a
													// Sprite ID
		} else {
	...
	/**
	 * Checks for collisions between the groups of Sprites specified in the
	 * constructor
	 */
	@Override
	 ...
	 * 
	 * @param a
	 *            A sprite being checked for collisions
	 * @param b
	 *            Being checked for collision with a
	 * @param data
	 *            Leveldata object
	 * @return whether A and B are colliding in the direction specified by the
	 *         user
	 */
	...
	/**
	 * Initializes the mapping of collision directions to numerical values
	 * returned by physics
	 */
